...
void KPromoteApp::initialize(int stage)
{
    if (stage == 0) {
        ...
        for (int id = 0; id <= getSimulation()->getLastComponentId(); id++) {
            ...
            nodeAddressList.push_back(nodeAddress);
        }
...
void KPromoteApp::handleMessage(cMessage *msg)
{
    ...
    if (msg->isSelfMessage() && msg->getKind() == 92) {
        ...
        if (destinationOriented) {
            ...
            list<string>::iterator ad = nodeAddressList.begin();
            advance(it, randomNodeIndex);
            advance(ad, randomNodeIndex); 
            string *nodeName = *it;
            dataMsg->setFinalDestinationNodeName(nodeName->c_str());
            dataMsg->setFinalDestinationNodeAddress(ad->c_str());
        ...
        KBaseNodeInfo *nodeInfo = new KBaseNodeInfo();
        nodeInfo->nodeModule = getParentModule();

        if((strstr(getParentModule()->getFullName(), dataMsg->getFinalDestinationNodeName()) != NULL) || (strstr(nodeInfo->nodeModule->par("ownAddress").stringValue(), dataMsg->getFinalDestinationNodeAddress()) != NULL)) {
