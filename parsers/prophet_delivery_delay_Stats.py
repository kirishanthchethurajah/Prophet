...
def main():
    ...
    for line in datafile:
        if "UI" in line and ("DM" in line):
            ...
            messageInfo['destinationReached'] = 0
        ...
        if "UO" in line and ("DM" in line):
            words = line.split(">!<")
            messageCheck = defaultdict(list)
            messageCheck = messages[words[8]]
            if words[3] == words[7]:
                if messageCheck['destinationReached'] == 0:
                    message_reached += 1
                    messageCheck['hopCount'] = words[11].strip()
                    messageCheck['destinationReached'] = 1
    ...
    for ke, va in message_generate_count.items():
        gen = 0
        rec = 0
        dest = 0 
    ...
    fi.write("Total:                                " + str(tot_message_generate_count) + "                            " + str(tot_message_destination_count)+"                                           " + str(tot_message_receive_count) + "\n")
    file.write("------------------------------------------------------")
    file.write("\nNo. of Messages generated : " + str(message_generated))
    file.write("\nNo. of Unique Messages transfered : " + str(len(message_transfer_count))) 
    file.write("\nNo. of Messages Reached : " + str(tot_message_receive_count))
    file.write("\nNo. of Messages yet to Receive : " + str(message_generated - tot_message_receive_count))
    file.write("\nAccumulated Delay " + str(accumulated_delay))
    file.write("\nAverage Delay " + str(accumulated_delay/tot_message_receive_count))
    ...
    file.write("\nDelivery Rate " + str((tot_message_receive_count * 100 )/message_generated))
